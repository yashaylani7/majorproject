...
class Home(QMainWindow):  # Home extends QMainWindow
    ...
    def home_page(self):
        ...
        self.setWindowTitle("Everyone Will Learn")  # Translated from "শিখবে সবাই"
    ...
    def student_page(self):
        ...
        # set window icon and title
        self.setWindowIcon(QIcon("Frontend/Images/primary_logo.png"))
        self.setWindowTitle("Student Information")  # Translated from "শিক্ষার্থীর তথ্য"
    ...
    def lesson_page(self):
        ...
        os.path.exists('Lessons/Modules') or os.mkdir('Lessons/Modules')  # Translated from "মডিউলসমূহ"
        os.path.exists('Lessons/Lessons') or os.mkdir('Lessons/Lessons')  # Translated from "পাঠসমূহ"
        ...
        # set window icon and title
        self.setWindowIcon(QIcon("Frontend/Images/primary_logo.png"))
        self.setWindowTitle("Lesson Modules")  # Translated from "পাঠের মডিউলসমূহ"
    ...
    # ! TODO: Will move this method to separate class if we got time
    def sequence_page(self):
        ...
        if self.home.task_seq_video_frame_widget.count() != 0:
            self.home.task_seq_video_frame_widget.itemAt(0).widget().setParent(None)
    ...
    def save_sequence_details(self):
        ...
        if image_description == "" or image_set == "":
            show_warning_message("Field Incomplete", "Please fill in all fields")
            return
        ...
        if not os.path.exists('Lessons/Sequence_Images/{}'.format(image_set)) and not image_set in existing_folders:
            ...
            show_warning_message("Set Name Already Used",
                                 "A set with this name has already been used, please use a different name")
            return

        # check if 4 images are selected or not
        if self.sequence_image_count >= 3:
            ...
            show_confirmation_message(
                "Completed",
                "4 images have been selected for the {} set, you can add new images to another set".format(image_set))
            ...
            # count number of images for each set
            self.sequence_image_count += 1

            # save images to specific set
            shutil.move(current_saved_image_path, 'Lessons/Sequence_Images/{}/{}_{}.png'.format(
                image_set, self.sequence_image_count, image_description))
            show_warning_message("Image Selection Incomplete",
                                 "You need to select {} more images".format(4 - self.sequence_image_count))
    ...
    def select_puzzle_image(self):
        ...
        try:
            # Show file names in label
            names = ""
            for file_name in file_names:
                names += file_name.split("/")[-1] + "\n"

            names += "To save the images, enter the set number above and click the 'Save' button."
    ...
    def save_puzzle_set(self):
        ...
        # Rename folder from .temp
        if not os.path.exists('Lessons/Puzzle_Images/{}'.format(puzzle_set)):
            os.rename('Lessons/Puzzle_Images/.temp', 'Lessons/Puzzle_Images/{}'.format(puzzle_set))
            self.home.task_puzzle_image_lbl.setText(
                "Images have been saved in folder {}.\nPlease select new images to save to a different folder.".format(
                    self.home.task_puzzle_q_set_lbl.text())
            )

            self.home.task_puzzle_q_set_lbl.clear()
            show_success_message("Image save complete", "Save complete")

        else:
            show_warning_message("Folder already exists",
                                 "The folder {} already exists! Please choose a new set number.".format(
                                     self.home.task_puzzle_q_set_lbl.text()))
    ...
    # ! TODO: Will move this method to separate class if we have time
    ...
    def matching_process(self, frame_name):
        ...
        if not os.path.exists('Lessons/Matching_Images/.temp'):
        ...
        if dialog.exec_() == QFileDialog.Accepted:
            ...
            image_path = 'Lessons/Matching_Images/.temp/{}'.format(file_path.split('/')[-1])
            image_name = file_path.split('/')[-1]

            pixmap = QPixmap()
            pixmap.load(file_path)

            # Replace frame_name with actual frame
            ...
            # Rename the image
            try:
                os.rename('Lessons/Matching_Images/.temp/{}'.format(image_name),
                          'Lessons/Matching_Images/.temp/{}_{}.png'.format(image_name.split('.')[0],
                                                                           frame_name.objectName()[-1]))
    ...
    def save_matching_set(self):
        ...
        # Check if corresponding image descriptions are filled
        for item in self.current_matching_set:

            set_id = item.split('_')[-1].split('.')[0]

            if set_id == '1' and img_desc_1 == "":
                show_warning_message("Fill in the description",
                                     "Please provide a short description for image {}".format(set_id))
                return False
            elif set_id == '2' and img_desc_2 == "":
                show_warning_message("Fill in the description",
                                     "Please provide a short description for image {}".format(set_id))
                return False
            elif set_id == '3' and img_desc_3 == "":
                show_warning_message("Fill in the description",
                                     "Please provide a short description for image {}".format(set_id))
                return False
            elif set_id == '4' and img_desc_4 == "":
                show_warning_message("Fill in the description",
                                     "Please provide a short description for image {}".format(set_id))
                return False
        ...
        if matching_set_name == "":
            show_warning_message("Enter set name", "Please enter the set name")
            return False

        # Check if set name already exists
        if os.path.exists('Lessons/Matching_Images/{}'.format(matching_set_name)):
            show_warning_message("Rename the set", "A set with this name already exists")
            return False
        else:
        ...
        if set_items <= 1:
            show_warning_message("Not enough images", "The set must contain 4 images")
            shutil.rmtree('Lessons/Matching_Images/{}'.format(matching_set_name))
        else:
            show_success_message("Set saved successfully",
                                 "Set {} has been saved successfully".format(matching_set_name.split('_')[-1]))
    ...
    def create_mcq_set(self):
        ...
        if self.home.task_mcq_set_no_edit.toPlainText() == "":
            show_confirmation_message(
                "Set number missing", "Please provide a set number and click on 'Create New Set' button.")
            return False
        ...
        if os.path.exists(f"Lessons/MCQ_Questions/{set_no}"):
            ...
            show_warning_message(
                "Rename set", f"A set with this name already exists. The existing sets are: {sets}")
            return False
        else:
        ...
        show_confirmation_message(
            ...
            "Now provide the questions and possible answers. Click on 'Next Question' to create more questions.")
    ...
    def finish_mcq_set(self):
        ...
        # show message
        show_confirmation_message(
            "Set saved", f"MCQ set {set_no} has been saved. Click on 'Create New Set' to create a new question set.")
    ...
    def upload_mcq_image(self):
        ...
        img_path = QFileDialog.getOpenFileName(
            self, "Select an image", "", "Image Files (*.png *.jpg *.jpeg)")[0]
        if img_path != '':
            self.current_mcq_image_path = img_path
            shutil.copy2(
                img_path,
                f"Lessons/MCQ_Questions/{'q_' + self.home.task_mcq_set_no_edit.toPlainText()}//q_{self.total_mcq_questions}.png")
    ...
    def next_mcq_question(self):
        ...
        if question == "" or option_1 == "" or option_2 == "" or option_3 == "" or option_4 == "" or correct_answer == "":
            show_warning_message("Complete the question", "Please provide all the options.")
            return
    ...
    def load_user_performance_data(self, index):
        ...
        # if exists then we need to delete it first as we need new table for
    ...
    def load_surveillance_performance_data(self, sur_data):
        ...
        for item in sur_data:
            module_name = re.sub(r'^Learning\(|\)$', '', item[3])
    ...
    def load_performance_report(self):
        ...
        if self.home.performance_std_id_cmb.currentText() == "Select":
            show_warning_message("Student not selected", "Please select a student to view the performance report")
        ...
        try:
            ...
            show_confirmation_message("Please wait!", "Performance report is being generated")
